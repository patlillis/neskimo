...
impl<'a> CpuOperation<'a> {
    ...
    // No processor status flags are affected.
    pub fn php(&mut self) {
        let value = self.registers.p.0 | CpuFlags::B | CpuFlags::U;
        self.push(value);
    }
    ...
    pub fn plp(&mut self) {
        let value = self.pull();
        self.registers.p.0 = value & !CpuFlags::B & !CpuFlags::U;
    }
    ...
    fn rotate_l(&mut self, value: u8) -> u8 {
        let CpuFlags::C_bit = self.registers.p.0 & CpuFlags::C;
        let rotated_value = (value << 1) | CpuFlags::C_bit;
    ...
    fn rotate_r(&mut self, value: u8) -> u8 {
        let CpuFlags::C_bit = (self.registers.p.0 & CpuFlags::C) << 7;
        let rotated_value = (value >> 1) | CpuFlags::C_bit;
    ...
    pub fn bit(&mut self, address: u16) {
        ...
        self.registers.p.set_v(value & CpuFlags::V == CpuFlags::V);
        self.registers.p.set_n(value & CpuFlags::N == CpuFlags::N);
        self.decode_operand_value(value);
    }
    ...
    //         N    Negative Flag       Not affected
    pub fn brk(&mut self) {
        // Push return address and status onto stack. CpuFlags::U is 1, CpuFlags::B is 1.
        ...
        let vector = self.memory.fetch_u16(CpuVectors::IRQ);
    ...
    //         N    Negative Flag       Set from stack
    pub fn rti(&mut self) {
        let status = self.pull() & !(CpuFlags::B | CpuFlags::U);
