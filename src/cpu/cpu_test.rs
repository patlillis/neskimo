...
fn test_adc() {
    ...
    let adc_results = [
        (0x00, 0x00, false, 0x00, 0b00000010),
    ...
    for adc_result in adc_results.iter() {
        ...
        for _ in 0..adc_addresses.len() + 1 {
            ...
            assert!(
                cpu.registers.p.0 == adc_result.4,
                "Bad flag: {:08b}",
                cpu.registers.p.0
            );
        }
...
fn test_and() {
    ...
    for and_result in and_results.iter() {
        ...
        for _ in 0..and_addresses.len() + 1 {
            ...
            assert!(
                cpu.registers.p.0 == and_result.3,
                "Bad flag: {:08b}",
                cpu.registers.p.0
            );
        }
...
fn test_asl() {
    ...
    for asl_result in asl_results.iter() {
        ...
        cpu.execute();
        assert!(
            cpu.registers.p.0 == asl_result.1,
            "Bad flags after ASL on accumulator: {:08b}",
            cpu.registers.p.0
        );
        assert!(
            cpu.registers.a == (asl_result.0 << 1),
...
fn test_cmp() {
    ...
    for cmp_result in cmp_results.iter() {
        ...
        for _ in 0..8 {
            ...
            cpu.execute();
            assert!(
                cpu.registers.p.0 == cmp_result.1,
                "Bad flag: {:08b}",
                cpu.registers.p.0
            );
        }
...
fn test_cpx() {
    ...
    for cpx_result in cpx_results.iter() {
        ...
        for _ in 0..3 {
            ...
            cpu.execute();
            assert!(
                cpu.registers.p.0 == cpx_result.1,
                "Bad flag: {:08b}",
                cpu.registers.p.0
            );
        }
...
fn test_cpy() {
    ...
    for cpy_result in cpy_results.iter() {
        ...
        for _ in 0..3 {
            ...
            cpu.execute();
            assert!(
                cpu.registers.p.0 == cpy_result.1,
                "Bad flag: {:08b}",
                cpu.registers.p.0
            );
        }
...
fn test_dec() {
    ...
    for addr in addresses.iter() {
        ...
        cpu.execute();
        assert!(
            val - 1 == cpu.memory.fetch(*addr as u16),
...
fn test_eor() {
    ...
    for eor_result in eor_results.iter() {
        ...
        for _ in 0..eor_addresses.len() + 1 {
            ...
            assert!(
                cpu.registers.a == eor_result.2,
                "Bad result {:#04x} in A after EOR",
                cpu.registers.a
            );
            assert!(
                cpu.registers.p.0 == eor_result.3,
                "Bad flag: {:08b}",
                cpu.registers.p.0
            );
        }
...
fn test_flags() {
    ...
    cpu.memory.store_bytes(
        ...
        &[
            // Carry flag
            SEC as u8, CLC as u8, // Interrupt flag
            SEI as u8, CLI as u8, // Overflow flag
            CLV as u8, // Decimal flag
            SED as u8, CLD as u8,
        ],
    );
...
fn test_jmp() {
    ...
    // Jump to 0x1234.
    cpu.memory.store_bytes(0x0000, &[JMP_Abs as u8, 0x34, 0x12]);

    // Jump to 0x2fff (thru 0x4545).
    cpu.memory.store_bytes(0x1234, &[JMP_Ind as u8, 0x45, 0x45]);

    // Jump to 0x00fa.
    cpu.memory.store_bytes(0x2fff, &[JMP_Abs as u8, 0xfa, 0x00]);

    // Jump to 0xfedc (thru 0x3fff).
    cpu.memory.store_bytes(0x00fa, &[JMP_Ind as u8, 0xff, 0x3f]);
...
fn test_lsr() {
    ...
    for lsr_result in lsr_results.iter() {
        ...
        cpu.execute();
        assert!(
            cpu.registers.p.0 == lsr_result.1,
            "Bad flags after LSR on accumulator: {:08b}",
            cpu.registers.p.0
        );
        assert!(
            cpu.registers.a == (lsr_result.0 >> 1),
            "Bad shift result {:#04x} in accumulator",
            cpu.registers.a
        );
...
fn test_ora() {
    ...
    for ora_result in ora_results.iter() {
        ...
        for _ in 0..ora_addresses.len() + 1 {
            ...
            assert!(
                cpu.registers.a == ora_result.2,
                "Bad result {:#04x} in A after ORA",
                cpu.registers.a
            );
            assert!(
                cpu.registers.p.0 == ora_result.3,
                "Bad flag: {:08b}",
                cpu.registers.p.0
            );
        }
...
fn test_registers() {
    ...
    cpu.memory.store_bytes(
        ...
        &[
            // X register
            TAX as u8, TXA as u8, DEX as u8, INX as u8,
            // Y register
            TAY as u8, TYA as u8, DEY as u8, INY as u8,
        ],
    );
...
fn test_rol() {
    ...
    for rol_result in rol_results.iter() {
        ...
        cpu.execute();
        assert!(
            cpu.registers.p.0 == rol_result.3,
            "Bad flags after ROL on accumulator: {:08b}",
            cpu.registers.p.0
        );
        assert!(
            cpu.registers.a == (rol_result.2),
...
fn test_ror() {
    ...
    for ror_result in ror_results.iter() {
        ...
        cpu.execute();
        assert!(
            cpu.registers.p.0 == ror_result.3,
            "Bad flags after ROR on accumulator: {:08b}",
            cpu.registers.p.0
        );
        assert!(
            cpu.registers.a == (ror_result.2),
            "Bad rotate result {:#04x} in accumulator",
            cpu.registers.a
        );
...
fn test_sbc() {
    ...
    for sbc_result in sbc_results.iter() {
        ...
        for _ in 0..sbc_addresses.len() + 1 {
            ...
            assert!(
                cpu.registers.a == sbc_result.3,
                "Bad result {:#04x} in A after SBC",
                cpu.registers.a
            );
            assert!(
                cpu.registers.p.0 == sbc_result.4,
                "Bad flag: {:08b}",
                cpu.registers.p.0
            );
        }
